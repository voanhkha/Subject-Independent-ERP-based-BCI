...
global C ind deps Q Rs X Y scale type
...
% C = model.C; 		% regularization parameters
% ind = model.ind;    % cell array containing indices of margin, error, reserve and unlearned vectors
% % deps = model.deps;   % jitter factor in kernel matrix
% Q = model.Q;      % extended kernel matrix for all vectors
% Rs = model.Rs;     % inverse of extended kernel matrix for margin vectors   
% %scale = model.scale;  % kernel scale
% %type = model.type;   % kernel type
% X = model.X;      % matrix of margin, error and reserve vectors stored columnwise
% Y = model.Y; 
